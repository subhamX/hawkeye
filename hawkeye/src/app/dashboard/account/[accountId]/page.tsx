...
export default async function AccountDashboardPage({ params }: AccountDashboardPageProps) {
  ...
  const dashboardData = await dashboardService.getAccountDashboard(params.accountId, session.user.id);
  
  if (!dashboardData) {
    notFound();
  }

  const { account, analysisHistory, totalPotentialSavings, lastRunDate, servicesBreakdown, recommendations } = dashboardData;
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {Object.entries(servicesBreakdown).map(([service, config]) => (
                ...
                {analysisHistory.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Run your first analysis to start getting cost optimization recommendations.
                    </p>
                    <Button asChild>
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          {run.totalSavings && run.totalSavings > 0 && (
                            <div className="text-right">
                              ...
                              <p className="text-xs text-muted-foreground">savings found</p>
                            </div>
                          )}
                          <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/account/${account.id}/analysis/${run.id}`}>
                              View Details
                            </Link>
                          </Button>
                        </div>
